...
# Model
model = modelsource.r2plus1d_18()
print(model)
model = model.to(device)

# Data
# Transforms
train_transform=1 # Scipy ndimage rotate, affine, normalization
test_transform=None
val_transform=None
# Dataset
...
# Optimizer
optimizer = torch.optim.SGD(model.parameters(), lr=3e-4, weight_decay=1e-4, momentum=0.9)
...
def train(model, trainloader, valloader, optimizer, scheduler):
    for epoch in range(num_epochs):
        ...
        for batch_idx, (vid, ef) in tqdm(enumerate(trainloader)):
            ...
            video = vid.to(device)
            EF = ef.to(device)
            pred_l2, predicted_EF = model(video)
            # Step
            loss = mseloss(predicted_EF, EF)
            l2_loss = mseloss(pred_l2, EF)
            print(loss)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (vid, ef) in tqdm(enumerate(valloader)):
                video = vid.to(device)
                EF = ef.to(device)
                _, predicted_EF = model(video)
                loss = mseloss(predicted_EF, EF)
                running_mseloss += loss.item()
                running_rmseloss += math.sqrt(loss.item())
                running_maeloss += maeloss(predicted_EF, EF)
            # Save data for plotting curves
            train_loss = running_mseloss/len(valset)
            rmse_loss = running_rmseloss/len(valset)
            mae_loss = running_maeloss/len(valset)
            # log
...
with torch.no_grad():
    model.eval()
    running_mseloss = 0.0
    for batch_idx, (vid, ef) in tqdm(enumerate(testloader)):
        video = vid.to(device)
        EF = ef.to(device)
        _, predicted_EF = model(video)
        loss = mseloss(predicted_EF, EF)
        running_mseloss += loss.item()
    running_mseloss /= len(testset)
print("FINAL LOSS")
print(running_mseloss)
